...
app.get('/og', async (c) => {
  ...
  const params: OGParams = {
    title,
    description:
      (q['description'] ?? '').trim().slice(0, 200) || undefined,
    ...
    template: (
      ['blog', 'article'].includes(q['template'] ?? '')
        ? q['template']
        : 'default'
    ) as OGParams['template'],
  };
...
app.post('/register', async (c) => {
  ...
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    return htmlResponse(
      registerPage('Please enter a valid email address', tier),
      400
    );
  }

  const validTiers: Tier[] = ['free', 'pro', 'business'];
  const safeTier: Tier = validTiers.includes(tier as Tier)
    ? (tier as Tier)
    : 'free';
  ...
  if (!user) {
    return htmlResponse(
      registerPage('Database error — please try again'),
      500
    );
  }
